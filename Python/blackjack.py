...
def new_deal():
	...
	'''
	global deck, dealer_hand, dealer_card, hand, num, count, player_wins, dealer_wins, bet, balance
...
def pass_card(double=False):
	...
	if dealer > 21 and player <= 21:
		...
		configure(seven)
		if double == True:
			balance += int(new_bet * 2)
			configure(twelve, False, False)
		else:
			balance += bet
			configure(twelve)
		configure(ten)
...
def configure(option, minus=False, bet_option=True):
	global new_bet, bet
	if minus == False and bet_option == False:
		winnings = int(new_bet * 2)
	elif minus == True and bet_option == False:
		winnings = int(new_bet)
	else:
		winnings = bet
	if option == seven:
		...
		if minus == False:
			twelve.configure(text='Game winnings: ' + str(winnings), font=('Helvetica', '12'))
		else:
			twelve.configure(text='Game winnings: ' + str(-winnings), font=('Helvetica', '12'))
...
Pass = tk.Button(root, text='Stay', font=('Helvetica', '12', 'bold'), command=pass_card) 
