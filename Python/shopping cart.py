...
def display_cart_qty(cartQty, products, prices):
    ...
    for number in cartQty:
        if int(number) > 0:
            i_show = format(number, '3d')
            price_show = format(prices[number-1], '10,.2f')
            print(i_show, fixed_width(products[number-1], 16, '.'), price_show, price_show*number)
...
if action == 'c' and total_owed > 0:
    print("\nChecking out, please pay for your shop cart contents\n")
